...
const CurrentEditing = () => {
  ...
  useEffect(() => {
    const fetchTest = async () => {
      try {
        const response = await axios.get("https://ebc-training-and-research-center.onrender.com/currents/editing");
  ...
  const sendToTelegram = async (data) => {
    const telegramBotToken = "7571054173:AAEEAJrTYTLCbhEGLEf_evw3zqVMRWIOu9s";
    const chatId = "1262984155";
  
    // Create a formatted message string without correct answers
    const message = `
      Editing and Graphics Test
      Name: ${data.name}
      Class: ${data.class}
      ID No: ${data.idNo}
    ...
    try {
      await axios.post(`https://api.telegram.org/bot${telegramBotToken}/sendMessage`, {
        chat_id: chatId,
        text: message.trim(), // Remove any extra spaces
      });
  ...
  const handleSubmitTest = async () => {
    ...
    await sendToTelegram(submissionData); // Send to Telegram
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center">
      {!isAuthenticated ? (
        ...
        <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-4xl">
          <h3 className="text-2xl font-bold text-center mb-6">Eding and Graphics Test</h3>
          {testData.map((question, index) => (
            <div key={index} className="mb-6">
              <p className="font-semibold text-lg">
              {question.no}. {question.questionText} 
              </p>

...
const CurrentIt = () => {
  ...
  useEffect(() => {
    const fetchTest = async () => {
      try {
        const response = await axios.get("https://ebc-training-and-research-center.onrender.com/currents/it");
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center">
      {!isAuthenticated ? (
        ...
        <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-4xl">
          <h3 className="text-2xl font-bold text-center mb-6">Brodcast IT Test</h3>
          {testData.map((question, index) => (
            <div key={index} className="mb-6">
              <p className="font-semibold text-lg">
              {question.no}. {question.questionText}                 </p>

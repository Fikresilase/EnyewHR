...
const navigate=useNavigate();
  useEffect(() => {
    ...
    axios
      .get("https://ebc-training-and-research-center.onrender.com/tests/camera")
      .then((response) => {
  ...
  const handleQuestionSelect = (question) => {
    if (selectedQuestions.some((q) => q.no === question.no)) {
      ...
      setSelectedQuestions((prev) =>
        prev.filter((q) => q.no !== question.no)
      );
    } else {
  ...
  const submitSelectedQuestions = () => {
    ...
    axios
      .post("https://ebc-training-and-research-center.onrender.com/currents/camera", {
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      <div className="container pb-6 justify-between flex">
        <h1 className="text-3xl px-10 font-bold mb-4">
          Select Questions for camera Test
        </h1>
        <button

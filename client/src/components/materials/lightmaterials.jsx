...
const LightMaterials = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("https://ebc-training-and-research-center.onrender.com/materials/light");
        if (!response.ok) {
  ...
  const handleDelete = async (number) => {
    ...
    try {
        ...
        const response = await fetch(
            `https://ebc-training-and-research-center.onrender.com/materials/light/${number}`,
            {
                method: "DELETE",
                headers: {
  ...
  return (
    <div className="p-4 relative">
      ...
      <h1 className="text-center text-2xl font-bold mb-4">Light Materials</h1>

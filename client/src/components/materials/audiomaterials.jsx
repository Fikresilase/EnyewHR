...
const AudioMaterials = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("https://ebc-training-and-research-center.onrender.com/materials/audio");
        if (!response.ok) {
  ...
  const handleDelete = async (number) => {
    ...
    try {
        ...
        const response = await fetch(
            `https://ebc-training-and-research-center.onrender.com/materials/audio/${number}`,
            {
                method: "DELETE",
                headers: {
  ...
  return (
    <div className="p-4 relative">
      ...
      <h1 className="text-center text-2xl font-bold mb-4">Audio Materials</h1>
